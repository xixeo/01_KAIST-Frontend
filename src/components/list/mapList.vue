<template>
  <!-- HTML 정의하는 부분 -->
  <!-- 1차 퍼블리셔가 작업 영역 -->
  <!-- 2차 개발자가 Template 화 작업 진행-->
  <!--현대미포 맵 선택-->
  <div class="maplist">
    <ul v-if="listInfo.length>0">
      <li v-for="(item, index) in listInfo" :key="index" @click="clickItem(item, index)">
        <a href="#">
          <div class="title" :style="!item.isExpandable?'margin-bottom: 5px':''">{{item.title}}</div>
          <div class="map" v-if="item.isExpandable">
            <img :src="item.map" width="82%">
          </div>
        </a>
      </li>
    </ul>
  </div>
</template>

<script>
// 스크립트를 정의하는 부분
// 개발자 작업 영역

import _ from 'lodash'

// [ECMA6] export default 된 부분이 외부에서 import로 사용할 수 있게 된다.
export default {
  name: 'mapList',
  components: {
    //
  },
  props: {
    items: {
      type: Array,
      default: () => {
        return []
      }
    }
  },
  // .vue 내부에서 사용되는 model
  // model 기반으로 vue는 동작된다
  data() {
    return {
      selected: 1,
      // fits: ['fill', 'contain', 'cover', 'none', 'scale-down'],
      // 이미지 정보
      listInfo: []
    }
  },
  watch: {
    // 아이템 리스트 갱신하면 데이터 복사 및 갱신
    items() {
      this.makeItems()
    }
  },
  created() {},
  mounted() {
    this.makeItems() // 화면에 출력할 데이터를 Merge
  },
  beforeDestroy() {
    this.listInfo = []
  },
  methods: {
    // .vue 내부에서 사용되는 함수를 정의한다.
    makeItems() {
      this.listInfo = []
      this.items.forEach(item => {
        this.listInfo.push(item)
      })
      console.log('items change', this.listInfo)
    },
    // 클릭하면 호출
    clickItem(item, index) {
      item.isExpandable = !item.isExpandable
      this.selected = index
      // console.log(item, index)
      this.$emit('clickItem', item) // 정보를 부모에게 전송
    },
    checkExpandable(item) {
      // console.log('checkExpandable', item.isExpandable)
    }
  }
}
</script>
<style lang="scss" scoped>
$textColor: white;
$bgColor: #151619;
$hoverColor: #1a7dd2;
$boxColor: #4f5358;

.maplist {
  overflow: hidden;
  li {
    width: 100%;
    --background: $bgColor;
    border: 1px solid $boxColor;

    div.title {
      float: left;
      width: 100%;
      height: 22px;
      padding: 0% 0px;
      background: $boxColor;
      color: $textColor;
      text-indent: 5%;
    }
    div.map {
      float: left;
      width: 100%;
      height: 122px;
      text-align: center;
      color: $textColor;
      background: $bgColor;
      margin-bottom: 5px;
    }
    :hover {
      div.title {
        background: $hoverColor;
      }
      // background: #1a7dd2;
    }
  }
}
</style>
