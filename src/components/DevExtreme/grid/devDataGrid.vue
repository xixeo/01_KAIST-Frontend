<template>
  <div>
    <div class="d-flex-grid">
      <!-- Grid Title -->
      <div v-if="gridInfo.title" class="col-sm-6 text-white m-1 mb-1" style="font-size:20px">
        <!-- <i class="fas fa-info-circle mr-1"></i> -->
        {{ gridInfo.title }}
      </div>
    </div>
    <div>
      <!-- 2021-10-15 by lyj focused-row-index 추가 -->
      <DxDataGrid
        id="gridContainer"
        :ref="this.gridRef"
      ...
      >
        ...
        <DxColumn
          v-for="(item, index) in gridInfo.gridColinfos"
          :key="index"
          ...
          :cell-template="
            item.button == true
              ...
              ? 'statusImage'
              : item.displayRowNumber == true
              ? 'DisplayRowNumberOfGrid'
              : null
          "
          :edit-cell-template="
        ...
        <template #buttonEditPopup="{ data: cellInfo }">
          <GridButton
            :visible="buttonVisible(cellInfo)"
            :value="cellInfo.value"
            :cellInfo="cellInfo"
            :buttonText="buttonText(gridInfo)"
            :icon="icon(gridInfo)"
            @btnClick="btnClick(cellInfo)"
          />
        </template>
        <!-- 2021-10-08 BY LYJ -->
        <!-- use image in cell -->
        <template #statusImage="{ data: cellInfo }">
          <div>
            <img :src="require(cellInfo.value == true ? '@/assets/images/status/hosun_img02.png' : '@/assets/images/status/hosun_img03.png')" />
          </div>
        </template>
        <!-- 2021-10-01 BY LYJ -->
        <!-- display row number -->
...
export default {
  ...
  methods: {
    ...
    buttonText(gridInfo) {
      if (gridInfo.buttonText != null) {
        return gridInfo.buttonText
      }
    },
    buttonVisible(cellinfo) {
      console.log('visible : ' + cellinfo.value)
      return cellinfo.value
    },
    icon(gridInfo) {
      return gridInfo.icon
    },
    focusedRowKey(rowHandle) {
      // var dataGrid = this.$refs[this.dataGridRefName].instance
      // //this.$refs[this.dataGridRefName].selectRows([rowHandle])
      // dataGrid.selectRows([0])

      this.dataGrid.selectRowsByIndexes(0)
    },
    setKeyindex() {
      // let i = 0
    ...
    btnClick(e) {
      console.log('devDtaaGrid.vue => ' + this.routeName + '_' + this.gridInfo.gridName + '_btnClick')
