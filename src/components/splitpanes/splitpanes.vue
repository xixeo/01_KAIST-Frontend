      ...
      if (dragPercentage > 100 - sums.nextPanesSize - this.panes[panesToResize[1]].min) {
        ...
        if (panesToResize[1] === undefined) {
          ...
          this.panes[panesToResize[0]].size =
            100 - sums.prevPanesSize - sums.nextReachedMinPanes - this.panes[this.panesCount - 1].min - sums.nextPanesSize
          return null
        }
      }
...
<style lang="scss">
@import '../../assets/scss/styles/default/variables.scss';
...
.splitpanes.default-theme {
  .splitpanes__pane {
    ...
    border: 1px solid $content-box_border;

    &:hover {
      box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.176) !important;
    }
  }
...
.default-theme {
  ...
  .splitpanes--vertical > .splitpanes__splitter {
    ...
    border-left: 1px solid $content-box_border;
    margin-left: -1px;
    &:before,
    &:after {
